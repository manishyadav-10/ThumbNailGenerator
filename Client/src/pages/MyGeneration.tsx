...
const MyGeneration = () => {
  ...
  const fetchThumbnails = async () => {
     try{
      ...
      // finally block will execute regardless of try/catch outcome (ye chalega chahe error aaye ya na aaye)
     finally{
      setLoading(false);
     }
  }
  ...
  return (
    ...
    <div className="mt-32 min-h-screen px-6 md:px-16 lg:px-24 xl:px-32">
      ...
      {/* Thumbnails  grid */}
      {!loading && thumbnails.length > 0 && (
        <div className="columns-1 sm:columns-2 lg:columns-4 2xl:columns-4 gap-8">
                  ...
                  <div className={`relative overflow-hidden rounded-t-2xl ${aspectClass} bg-black`}>
                    {thumb.image_url ?(
                      ...
                      <div className="w-full h-full flex items-center justify-center text-sm text-zinc-400">
                        {thumb.isGenerating ? 'Generating...' : 'Failed to generate image'}
                      </div>
                    )}
