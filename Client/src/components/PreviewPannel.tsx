        ...
        <div className={`relative overflow-hidden ${aspectClasses[aspectRatio]}`}> 
            ...
            {isLoading && (
                <div className="absolute inset-0 flex flex-col items-center justify-center gap-4 bg-black/25">
                    <Loader2Icon className="size-2 animate-spin text-zinc-400"/>
            ...
            {!isLoading && thumbnail?.image_url && (
                <div className="group relative h-full w-full">
                    ...
                    <button onClick={onDownload} type="button" className="mb-6 flex items-center gap-2 rounded-md px-5 py-2.5 text-xs
                    font-medium transition bg-white/30 ring-2 ring-white/40 backdrop-blur hover:scale-105 active:scale-95">
